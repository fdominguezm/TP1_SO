...
void read_and_send_files(slave slaves[], int num_task,int num_slaves, FILE * result_file, char *const argv[], FILE * n_pipe);
...
int main (int argc, char *argv[]) {
    ...
    if (mkfifo(N_PIPE, S_IWUSR) == -1) perror ("application: n_pipe create");
    ...
    FILE * n_pipe = fopen(N_PIPE, "w"); //Creo el archivo result para enviarselo a los slaves
    if (n_pipe == NULL) perror("Result File Failed");  

    sleep(2); //Espero a que comience el proceso vista

    create_slaves(slaves, num_slave, argv); //Creo los esclavos
    read_and_send_files(slaves, num_task, num_slave, result_file, argv, n_pipe); // Proceso todos los archivos y se los envio a los esclavos
    ...
    if (fclose(n_pipe) == -1) perror("close n_pipe");
    ...
    while (finished_tasks < num_task) {
        ...
        for(int i = 0; i < num_slaves;i++ ) {
            if(slaves[i].active && FD_ISSET(slaves[i].fd_out,&read_fdSet)){
                ...
                fprintf(n_pipe,"%d %s",slaves[i].pid, buffer);
                // int position = (finished_tasks)*BUFFER_SIZE;
                // char str[8];
                // sprintf(str, "%d", slaves[i].pid);
                // write(n_pipe, str, BUFFER_SIZE);
